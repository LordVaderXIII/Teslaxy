...
## 2026-01-26 - Unbounded Transcoding Concurrency (DoS)
**Vulnerability:** The transcoding endpoint allowed unlimited concurrent FFmpeg processes, potentially exhausting CPU/RAM and crashing the server (Denial of Service).
**Learning:** External processes (like FFmpeg) are heavy resources. Launching them directly in response to user requests without a semaphore allows trivial DoS attacks.
**Prevention:** Implemented a buffered channel semaphore (`transcodeSemaphore`) to strictly limit concurrent execution of resource-heavy tasks to 4 active sessions, returning 503 Service Unavailable when saturated.
