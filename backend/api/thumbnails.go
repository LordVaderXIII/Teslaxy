...
func getThumbnail(c *gin.Context) {
	...
	if err != nil || width < 32 || width > 1920 {
		width = 480
	}
	...
	// Hash the full path AND the seekTime AND width to ensure uniqueness
	...
	// -vf scale=WIDTH:-1: resize to specified width (keep aspect ratio)
	// -q:v 5: quality (1-31, lower is better)
	scaleFilter := fmt.Sprintf("scale=%d:-1", width)
	cmd := exec.Command("ffmpeg", "-y", "-ss", seekTime, "-i", fullPath, "-vframes", "1", "-vf", scaleFilter, "-q:v", "5", thumbPath)
