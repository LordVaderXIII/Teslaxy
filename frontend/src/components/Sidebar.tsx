       ...
       <div className="flex-1 min-w-0">
          <div className="flex justify-between items-baseline mb-1">
             ...
             <span className="text-xs text-gray-500 font-mono">
                 {(clip.start_time || new Date(clip.timestamp)).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
             </span>
          </div>
...
const Sidebar: React.FC<SidebarProps> = ({ clips, selectedClipId, onClipSelect, onRefresh, loading, className }) => {
  ...
  const clipsByDate = useMemo(() => {
    ...
    clips.forEach(clip => {
      // Bolt Optimization: Use cached date key if available to avoid repeated parsing
